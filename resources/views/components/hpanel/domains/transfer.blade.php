...
<section class="reveal">
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    {{-- Header --}}
    <div class="mb-5">
      <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">Transfer domain</h1>
      <p class="mt-1 text-sm text-gray-600">Move a domain you already own to us.</p>
    </div>
    ...
    <form method="GET" action="{{ route('domains.transfer') }}"
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm text-gray-600 mb-1">Domain name</label>
          <input name="domain" value="{{ $domain ?? '' }}" placeholder="yourdomain.com"
                 class="w-full h-11 rounded-xl px-3 bg-gray-50
                        border border-gray-200 text-sm
                        text-gray-800 placeholder-gray-400">
        </div>
        <div>
          <label class="block text-sm text-gray-600 mb-1">Auth/EPP code</label>
          <input placeholder="EPP-XXXXXXXX" class="w-full h-11 rounded-xl px-3 bg-gray-50
                        border border-gray-200 text-sm
                        text-gray-800 placeholder-gray-400">
        </div>
      </div>
...
<div class="mt-4 flex justify-end gap-3">
  ...
  <button type="submit"
    ...
    {{-- Tips --}}
    <div class="mt-6 rounded-2xl border border-gray-200 bg-white p-4 sm:p-6">
      <h3 class="font-semibold text-gray-900">Before you start</h3>
