...
<header x-data="header()" x-init="init()"
    class="fixed top-0 inset-x-0 z-50 border-b border-gray-200
         bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/90
         text-gray-900">
    ...
    <div class="mx-auto max-w-full h-16 px-3 sm:px-6 lg:px-8 grid grid-cols-3 items-center gap-2">
        ...
        <div class="flex items-center gap-2">
            ...
            <button @click="window.dispatchEvent(new CustomEvent('toggle-sidebar'))"
                class="p-2 rounded-xl hover:bg-gray-100 focus:outline-none"
        ...
        <div class="ml-auto flex items-center justify-end gap-1.5">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open=!open" class="p-2 rounded-xl hover:bg-gray-100 relative">
                ...
                <div x-show="open" x-transition @click.outside="open=false"
                    class="absolute right-0 mt-2 w-72 rounded-xl border border-gray-200
                    bg-white shadow-lg overflow-hidden">
                    <div class="px-3 py-2 text-xs font-semibold text-gray-600">Notifications</div>
                    <ul class="max-h-64 overflow-y-auto text-sm">
                        <li class="px-3 py-2 hover:bg-gray-50">Domain transfer approved</li>
                        <li class="px-3 py-2 hover:bg-gray-50">Invoice #324 paid</li>
                        <li class="px-3 py-2 hover:bg-gray-50">Security tip: enable 2FA</li>
                    </ul>
                    <a href="#"
                        class="block px-3 py-2 text-xs text-indigo-600 hover:underline">View
                        all</a>
                </div>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-transition @click.outside="open=false"
                    class="absolute right-0 mt-2 w-56 rounded-xl border border-gray-200
                    bg-white shadow-lg overflow-hidden text-sm">
                    <a href="#" class="block px-3 py-2 hover:bg-gray-50">My profile</a>
                    <a href="#" class="block px-3 py-2 hover:bg-gray-50">Billing</a>
                    <hr class="border-gray-200/70">
                    <form method="POST" action="#">
                        @csrf
                        <button class="w-full text-left px-3 py-2 hover:bg-gray-50">Sign
                            out</button>
                    </form>
                </div>
            </div>
    ...
    <div x-show="openSearch" x-transition.opacity class="fixed inset-0 z-50" aria-modal="true" role="dialog">
        ...
        <div
            class="relative mx-auto mt-24 w-[92%] max-w-xl rounded-2xl border border-gray-200
                bg-white shadow-2xl overflow-hidden">
            <div class="flex items-center gap-2 px-3 h-12 border-b border-gray-200">
                <i data-lucide="search" class="h-4 w-4 opacity-80"></i>
                <input x-ref="search" id="global-search" type="text" placeholder="Type to search…"
                    class="flex-1 bg-transparent outline-none text-sm
                      placeholder-gray-400 text-gray-900" />
                <button class="p-1.5 rounded-lg hover:bg-gray-100" @click="openSearch=false">
                    <i data-lucide="x" class="h-4 w-4"></i>
                </button>
            </div>
            <ul class="py-2 max-h-80 overflow-y-auto text-sm">
                <li class="px-3 py-2 hover:bg-gray-50 cursor-pointer">Domains → Portfolio
                </li>
                <li class="px-3 py-2 hover:bg-gray-50 cursor-pointer">Emails → Inboxes</li>
                <li class="px-3 py-2 hover:bg-gray-50 cursor-pointer">VPS → Instances</li>
            </ul>
        </div>
    </div>
</header>
